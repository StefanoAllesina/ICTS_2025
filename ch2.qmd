...
n \mathbb E((y_i)^2) + n (n-1)  \mathbb E( y_i y_j) &= \dfrac{n^2}{\alpha^2}\\
 \mathbb V(y_i) + \mathbb E(y_i)^2  + (n-1)\left( \mathbb{Cov}(y_i, y_j)  + \mathbb E(y_i^\star)  \mathbb E(y_j)  \right) &= n \mathbb E(y_i)^2\\
...
dt %>% ggplot(aes(x = alpha_F)) + stat_ecdf() + geom_function(fun = function(x) pnorm(x / sqrt(n-1))^n, colour = "red") 
```
...
n <- 250
sd_a <- function(a) sqrt(n-1)/a^2
dt %>% ggplot(aes(x = alpha_F)) + stat_ecdf() + geom_function(fun = function(x) pnorm(x / sqrt(n-1))^n, colour = "red") + geom_function(fun = function(x) pnorm(sqrt(x^2 / (n-1)-1))^n, colour = "darkgreen")
```
