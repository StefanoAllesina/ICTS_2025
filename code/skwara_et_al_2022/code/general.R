...
brute_force <- function(pars, E, Evar, D, replicates){
  tominimizek <- function(kpars, kpos, pars, E, Evar, D, replicates){
    pars[kpos] <- kpars
    return(minimize_given_pars(pars, E, Evar, D, replicates))
  }
  tmp <- list(par = pars)
  for (i in 1:6){
    tmp <- optim(par = tmp$par, fn = minimize_given_pars, E = E, Evar = Evar, D = D, replicates = replicates,
                 method = "Nelder-Mead",
                 control = list(maxit = 15000, trace = FALSE))
    tmp <- optim(par = tmp$par, fn = minimize_given_pars, E = E, Evar = Evar, D = D, replicates = replicates,
                 method = "BFGS",
                 control = list(maxit = 2500, trace = FALSE))
    # optimize 4 at a time
    print(tmp$value)
    for (j in 1:10){
      kpos <- sample(x = 1:length(tmp$par), 4)
      tmp2 <- optim(par = tmp$par[kpos], kpos = kpos, pars = tmp$par, fn = tominimizek, E = E, Evar = Evar, D = D, replicates = replicates,
                    method = "Nelder-Mead",
                    control = list(maxit = 15000, trace = FALSE))
      tmp$par[kpos] <- tmp2$par
    }
    tmp <- optim(par = tmp$par, fn = minimize_given_pars, E = E, Evar = Evar, D = D, replicates = replicates,
                 method = "Nelder-Mead",
                 control = list(maxit = 15000, trace = FALSE))
