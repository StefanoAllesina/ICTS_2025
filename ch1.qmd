...
is $x^\star = 0$, we say that the population is extinct at the equilibrium.
...
population survives, while the other goes extinct; the identity of the
population going extinct depends on the initial conditions).
...
showing that *the average density is* in fact *the equilibrium*. With a
similar argument, one can prove that if the trajectory stays in a
...
#| fig-width: 7
#| fig-height: 3
#| fig-cap: "Solid lines: trajectories; dashed lines: equilibrium values; dotted line: (cumulative) average of trajectory."
...
ggplot(dt1) + aes(x = time, y = value, colour = name) + geom_line() + geom_line(aes(y = avg), linetype = 3) + geom_hline(data = tibble(name = as.character(1:4), value = xstar), aes(yintercept = value, colour = name), linetype = 2) + theme_bw() + theme(legend.position = "none")
```
...
In certain models, we can show that whenever the populations start at a
...
stability of equilibria (we are simply changing the unit of measure we
Then,
...
set of coexisting populations are labeled $k = \{1, 2, \ldots, \|k\|\}$,
and the remaining species are labeled $\{\|k\|+1, \ldots, n\}$. We
partition the saturated equilibrium, the vector of growth rates and the
matrix of interactions accordingly:
...
We consider a modification of Goh's function:

$$
V(x) = \sum_{i \in k}\left(x_i - x_i^\star - x_i^\star \log \frac{x_i}{x_i^\star}\right) + \sum_{i \in n-k} x_i
$$

The derivative:

$$
\dot{V}(x) = \sum_i \left( \right) + 
$$
