...
Note that $A + A^T = 1 c^T + c 1^T$ is positive definite. To prove this, just consider XXXX

## Computing equilibria

For simplicity, we are going to consider the case in which $m_i = m$ for all $i$. Because the matrix is lower-triangular, computing the equlibria is especially simple, because each equilibrium depends only on those of the preceding species. The equilibrium of the first species is given by:
...
Thus, the first species follows the Levins' model, and persists as long as $c_1 > m$.

We compute the equilibrium for the second species, assuming that the first one is present:
...
Thus, we can compute the conditions for persistence recursively. We define $l_0 = m$, and then
