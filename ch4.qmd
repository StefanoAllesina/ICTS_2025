...
Note that $A + A^T = 1 c^T + c 1^T$ is positive definite. The matrix has rank $\leq 2$ (because it is the sum of two rank-one matrices). 


::: {.callout-note collapse="true"}
## Eigenvalues using traces

The trace of a matrix is the sum of its diagonal values:
...
$$
\text{Tr} A = \sum_i A_{ii} = \sum_i \lambda(A)_i
$$

The trace of the square of the matrix is the sum of the squared eigenvalues:

$$
\text{Tr} A^2 = \sum_i (A^2)_{ii} = \sum_i (\lambda(A)_i)^2
$$

This is true for all other powers.
:::

The sum of the two eigenvalues is $\lambda(A + A^T)_1 + \lambda(A + A^T)_2 = 2 \sum_i c_i$; call $s = \sum_i c_i$. The trace of the matrix squared $\text{Tr} (A + A^T)^2 = \sum_i \sum_j (c_i + c_j)^2 = $
